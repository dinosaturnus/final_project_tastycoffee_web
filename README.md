...
<a id="properties"></a>
## Properties-файлы
Для запуска тестов на локальной машинк необходимо создать в папке resources -> config следующие файлы:
...
Все тесты запустятся на вашей локальной машине с дефолтными настройками файла WebDriverConfig.
```bash
./gradlew clean test
```
